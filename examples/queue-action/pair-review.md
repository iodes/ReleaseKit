...
This example has used nine image-tool requests in total, including three for the independent theme treatment revision. Two required assets do not guarantee only two billable generations. The CLI itself made no image-service requests.
...
| Encoding | Two distinct, fully decoded PNG files: `dark-refined.png` and `light-refined.png` |
| Subject | Three list rows with one action revealed behind the middle row |
| Fixed alignment | The first row, neutral action backplate, and third row share a left boundary at approximately 17% of canvas width |
...
| Focal hierarchy | The neutral action tile has the strongest local value distinction; supporting thumbnails and bars stay quieter in both themes |
