...
export async function exportBundle(project: Project, current: string, options: { limit?: number; locale?: string; out: string }) {
  ...
  const bundle: Bundle = { schemaVersion: 1, currentVersion: current, locale: language, releases: [] };
  const copies: Array<{ src: string; relative: string }> = [];
  ...
  for (const copy of copies) await write(path.join(destination, copy.relative), await fs.readFile(copy.src));
